...
from dev.content.acc_diagnostics import build_link_reviews, load_catalog as load_acc_catalog, load_overrides
from dev.content.diagnostic_articles import load_catalog
...
def load_all_reviews(root: Path) -> tuple:
    reviewed = load_reviews(root / "dev/content/data/diagnostic-standard-links.json")
    acc_catalog = load_acc_catalog(root / "data/acc-diagnostics.json")
    acc_overrides = load_overrides(root / "dev/content/data/acc-standard-link-overrides.json")
...
def generate(root: Path, *, write: bool) -> tuple[int, int, int, bool, int]:
    ...
    autoformat_catalog = load_autoformat_catalog(root / "dev/content/data/autoformat-fixes.json")
