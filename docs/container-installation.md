...
в HTTP и stdio: capability `resources` отсутствует, все Resource methods
...
socket. Runtime хранит поисковый индекс, но не готовит дополнительные
полнокорпусные тексты для MCP Resources. Локальная нагрузочная проверка не доказывает
