...
Публичный `https://ai.v8std.ru/mcp` работает только с публичным индексом сайта и
не выполняет пользовательский код. При этом все параметры MCP-вызовов передаются
на сервер в HTTP body, поэтому публичный сервис не предназначен для приватных
фрагментов исходного кода в `v8std_explain_snippet`.

Для закрытого кода используйте локальный MCP-контейнер и передавайте на публичный
сервис только поисковые фразы, идентификаторы стандартов и коды диагностик.
Продакшен-контур должен держать MCP-процесс на `127.0.0.1`, проксировать его
через nginx и ограничивать `/mcp`: `client_max_body_size 256k`, rate limit и
логи без тел запросов.
...
MCP будет доступен только с этой машины:
...
С апреля 2026 публичные имена инструментов используют OpenAI-style
`lower_snake_case`: старые dotted-имена вида `v8std.search` заменены на
`v8std_search`. Это сохраняет namespace `v8std`, но убирает точку для клиентов,
которые валидируют имена как function/tool names OpenAI.
...
| `relations` | Необязательный список связей: `standard`, `diagnostic`, `edt_check`, `related` |
...
`v8std_explain_snippet` ориентирован на фрагменты кода. Для обычной фразы вроде
`модальные окна` используйте `v8std_search`.
...
В `docker-compose` порт опубликован как `127.0.0.1:8765:8765`, поэтому контейнер
слушает `0.0.0.0` только внутри Docker-сети и не открывает MCP наружу.
...
анализаторы и не принимает файлы исходников. Для анализа проекта подключайте локальный
