...
- `scripts/v8std_retrieval_rules.py`: полный analyze_snippet, bounded token preview,
  стабильная дедупликация идентичных signals; публичная форма analysis прежняя.
...
- [ ] Добавить regression на std485/std740 в конце 4000-символьной процедуры,
...
- [ ] Выполнить `.venv/bin/python -m unittest tests.test_v8std_mcp_snippet -v`:
...
- [ ] Реализовать отдельное разрешение целей и один query из preview. Для каждого
...
- [ ] Ограничить только выдаваемый token preview (целые токены, prefix), убрать
  идентичные signals через set ключей `(type, rule, value, tuple(target_ids))`.
...
- [ ] Добавить RED на constructor limits, N−1/N/N+1 для 4k/32k, два независимых
...
- [ ] Реализовать validator: type(value) is int, 4000 ≤ value ≤32000; private
...
- [ ] Добавить RED wire-test реального FastMCP через Starlette TestClient lifespan:
...
- [ ] Перед регистрацией установить вычисленную аннотацию
...
- [ ] До изменения config проверить текущий public unit через shlex/parse_args и
...
- [ ] Добавить Compose `V8STD_MCP_MAX_SNIPPET_CHARS: "${V8STD_MCP_MAX_SNIPPET_CHARS-4000}"`
  (пустое значение не подменять default), public ExecStart `--max-snippet-chars 4000`.
  Wrapper не дублирует parser и наследует env.
- [ ] Обновить docs: одна крупная процедура, default/local ceiling, CLI precedence,
...
- [ ] Smoke запущенных локальных Python и wrapper с временным свободным портом,
...
- [ ] Добавить позиционные случаи std485/std740/std415/UsingModalWindows в
...
- [ ] Написать RED unit-tests для расчёта p95/median-of-series и сравнения
...
- [ ] Измерить 20 warmups +200 samples ×3 series в одном процессе для короткого
...
- [ ] Саморевью diff по всем требованиям, включая конфигурацию, privacy, top-K,
...
- [ ] Выполнить architecture impact/validate --merge-ready, declared fitness,
...
Commit/merge выполняются после соответствующих проверок как интеграция, не
