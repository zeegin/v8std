...
`spec/` — корпус требований, решений, контрактов и планов v8std.
Он не входит в сайт, навигацию и AI-артефакты из `docs/`, но доступен в
публичном Git-репозитории. «Внутренний» здесь не означает «приватный».

Документы описывают устройство продукта и общие процедуры. Частные домены и
приложения владельца, адреса и идентификаторы машин, сведения об аккаунтах и
доступах, содержимое переписки и состояние конкретной эксплуатации в Git не
переносятся. Такие сведения остаются вне репозитория, в закрытой операторской
записи; в решении достаточно роли вроде «целевой сервер» или «сторонний сервис».
Публичные адреса самого продукта и технические интерфейсы описываются по делу.
Перед commit проверяется весь diff, включая документы и примеры, на частный
контекст. Если он уже попал в непубликовавшийся коммит, исправления только
следующим коммитом недостаточно: исходная запись останется в публикуемой истории.
