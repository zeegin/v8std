...
Нормативный процесс: [`process:architecture-artifacts@2`](process/architecture-artifacts-v2.md).
...
автоматической публикацией сайта, первичная активация и MCP deployment не являются
его checkbox-задачами.

После локального merge push проверенного `main` выполняется только по явному
запросу и автоматически публикует сайт. До первичной активации MCP deployment
требует отдельного запроса и точного SHA из `main`. После управляемой активации
разрешённый push запускает настроенный CI rollout проверенного main SHA и
опубликованного digest с post-deploy проверкой и rollback. Это не разрешает
агенту ручные изменения production, настроек или доступов вне этого пути.
