...
Одновременные old/new runtime плюс staging нового index должны поместиться
с запасом; иначе до включения rollout нужен более ёмкий host. Увеличение лимита
nginx не устраняет дефицит памяти или полосы.

На выделенном host остаются ai.v8std.ru, SSH, TLS renewal, защита и мониторинг.
...
ai/TLS/monitoring. Никакая стадия CI не удаляет посторонние сайты автоматически.
