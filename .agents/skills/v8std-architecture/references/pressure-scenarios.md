...
| «Сайт уже опубликовался после push — обнови заодно MCP» | Site и MCP deployment смешаны одним общим правилом | До первичной активации нужен отдельный запрос; после неё разрешённый push запускает только настроенный CI rollout проверенного SHA/digest, не ручные действия агента | Process v2: `AGENTS.md` и skill сохраняют границу initial activation и запрещают ad-hoc mutation |
