...
def edge_config(upstream):
    ...
    http = (ROOT / "deploy/nginx/http-v8std-mcp.conf").read_text()
    http = http.replace("worker_rlimit_nofile 131072;", "")
    http = http.replace("127.0.0.1:8765", upstream + ":8000")
    server = (ROOT / "deploy/nginx/server-v8std-mcp.conf").read_text()
    server = "\n".join(line for line in server.splitlines() if not any(
        token in line for token in ("listen [::]", "ssl_certificate", "access_log /var/log")))
