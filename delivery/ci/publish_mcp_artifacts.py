...
from delivery.index.generate_mcp_snapshot import _atomic_file
...
from delivery.vps.v8std_mcp_release import (
...
def input_identities(root, source_sha):
    ...
    dockerfile = read("delivery/mcp/Dockerfile").replace("\\\n", " ")
    copies = []
    ...
    runtime = expand(["delivery/mcp/Dockerfile", ".dockerignore", ".github/workflows/ci.yml", *copies])
    corpus = expand(["docs", "overrides", "delivery/site/Dockerfile", "delivery/site/site.conf", "zensical.toml", "retrieval-rules.yml", "LICENSE", "LICENSES",
                     "requirements-build.lock", "delivery/ci/Dockerfile", "delivery/ci/fonts.sha256", ".github/workflows/ci.yml",
                     "scripts/zensical_docs.sh", "scripts/zensical-version.sh",
                     "delivery/site/build_local_site.py",
                     ...
                     "delivery/index/generate_mcp_snapshot.py"])
    ...
    corpus -= {name for name in corpus if name.startswith(("docs/ai/", "docs/assets/social/"))
               or name in {"docs/llms.txt", "docs/llms-full.txt"}}
    pending = [name for name in corpus if name.endswith(".py")]
    inspected = set()
    while pending:
        name = pending.pop()
        ...
        for node in ast.walk(ast.parse(read(name))):
            ...
            for module in imports:
                paths = [module.replace(".", "/") + ".py",
                         "scripts/" + module.removeprefix("scripts.").replace(".", "/") + ".py"]
                for path in paths:
                    if path in tree and path not in corpus:
                        corpus.add(path)
                        pending.append(path)
    # Package initializers also affect imported build behavior.
    for name in list(corpus):
        if name.endswith(".py"):
            for parent in Path(name).parents:
                initializer = str(parent / "__init__.py")
                if initializer in tree:
                    corpus.add(initializer)
...
class CITransport:
    ...
    def tag(self, image, source_sha):
        ...
        if existing is not None:
            ...
            return
        fresh_context(Path(__file__).resolve().parents[2], os.environ)
...
def authorized(context):
    ...
    require(type(context["gates"]) is dict and GATES <= set(context["gates"]) <= GATES | {"architecture"}
