...
# The root-owned forced command is executed with Python -I.
# Add only its installation root, never cwd or an environment-provided path.
...
INSTALL = Path("/opt/v8std-release/delivery/vps/v8std_mcp_release.py")
...
def validate_publication_record(record, identity):
    """One bounded receipt schema, shared with operator publication handoff."""
    try:
        require(isinstance(record, dict) and {"header", "state"} <= set(record)
                <= {"header", "state", "cleanup_pending", "error_code"}, "publication_history")
        header = validate_upload(record["header"])
        require(header["publication_id"] == identity
                and record["state"] in {"RECEIVED", "VERIFIED", "RECOVERY_REQUIRED", "COMMITTED", "FAILED"},
                "publication_history")
        require("cleanup_pending" not in record or type(record["cleanup_pending"]) is bool, "publication_history")
        require("error_code" not in record or matches(re.compile(r"[a-z][a-z0-9_]{0,127}\Z"), record["error_code"]),
                "publication_history")
        return record
    except (ValueError, TypeError, KeyError):
        raise ReleaseError("publication_history") from None


def publication_history(root, *, deadline=None):
    """Read all receipts under release.lock; malformed history never lowers the watermark."""
    deadline = time.monotonic() + TRANSACTION if deadline is None else deadline
    directory = Path(root) / "publications"
    records, sequences = {}, set()
    if not os.path.lexists(directory):
        return records
    require(stat.S_ISDIR(directory.lstat().st_mode), "publication_history")
    for path in directory.iterdir():
        remaining(deadline)
        info = path.lstat()
        require(stat.S_ISREG(info.st_mode) and info.st_nlink == 1, "publication_history")
        # atomic() may die before rename. Its private, owned temporary file is
        # not a receipt, including when only partially written. Never adopt it.
        if re.fullmatch(r"\.write-[a-z0-9_]{8}", path.name):
            require(info.st_uid == os.geteuid() and stat.S_IMODE(info.st_mode) == 0o600
                    and info.st_size <= 128 * 1024, "publication_history")
            continue
        require(path.suffix == ".json" and matches(ID, path.stem), "publication_history")
        record = validate_publication_record(read_record(path), path.stem)
        sequence = record["header"]["sequence"]
        require(sequence not in sequences, "publication_history")
...
def validate_handoff_receipt(record):
    require(isinstance(record, dict) and set(record) ==
            {"schema_version", "handoff_sha256", "state", "publication_sequence"}
            and type(record["schema_version"]) is int and record["schema_version"] == 1
            and matches(HEX, record["handoff_sha256"]) and record["state"] in {"PREPARED", "COMMITTED"}
            and type(record["publication_sequence"]) is int
            and 0 <= record["publication_sequence"] <= 2**53 - 1, "handoff_receipt")
    return record


def publication_admission(root, records):
    marker = Path(root) / "handoff-import.json"
    if os.path.lexists(marker):
        record = validate_handoff_receipt(read_record(marker))
        require(record["state"] == "COMMITTED", "handoff_incomplete")
        sequences = {r["header"]["sequence"] for r in records.values()}
        require(record["publication_sequence"] in sequences, "handoff_history_missing")
    return max((r["header"]["sequence"] for r in records.values()), default=0)
...
def ingest(root, static_root, fd, *, seconds=30):
    ...
    with locked(root):
        ...
        records = publication_history(root, deadline=deadline)
        record = records.get(header["publication_id"])
        if record:
            require(record["header"] == header, "mutated_duplicate")
        else:
            require(header["sequence"] > publication_admission(root, records), "stale_sequence")
...
class Publisher:
    ...
    def publish(self, header):
        ...
        with locked(self.root):
            ...
            records = publication_history(self.root, deadline=time.monotonic() + TRANSACTION)
            require(header["publication_id"] in records, "publication_history")
            record = records[header["publication_id"]]
            require(record["header"] == header, "mutated_duplicate")
            ...
            if not verified:
                publication_admission(self.root, records)
                require(header["sequence"] > max((r["header"]["sequence"] for key, r in records.items()
                        if key != header["publication_id"]), default=0), "stale_sequence")
